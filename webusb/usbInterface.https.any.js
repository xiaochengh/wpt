...
usb_test(async () => {
  ...
  let configuration =
      new USBConfiguration(device, device.configurations[1].configurationValue);
...
usb_test(
    async () => {
      ...
      try {
        let usbInterface =
            new USBInterface(configuration, configuration.interfaces.length);
        assert_unreached(
            'USBInterface should reject an invalid interface number');
      } catch (error) {
...
usb_test(async () => {
  let {device} = await getFakeDevice(fakeBadDeviceInit);
  await device.open();
...
}, 'Return null with problematic device that has no any alternate interface');
